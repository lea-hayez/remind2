...
reportEmi <- function(gdx, output = NULL, regionSubsetList = NULL,
  ...
  vm_emiIndCCS_tibble <- vm_emiIndCCS[, , emiInd37_fuel] %>%
    as_tibble() %>%
    select(
      t = "ttot", regi = "all_regi", emiInd37 = "secInd37",
      vm_emiIndCCS = "value"
    ) %>%
  ...
  o37_demFeIndSub_tibble <- o37_demFeIndSub %>%
    as_tibble() %>%
    select(
      t = "ttot", regi = "all_regi", sety = "all_enty",
  ...
  pm_emifac_tibble <- pm_emifac %>%
    ...
    as_tibble() %>%
    select(
      t = "tall", regi = "all_regi", fety = "all_enty1",
      pm_emifac = "value"
    ) %>%
  ...
  subsector_emissions <- inner_join(
    ...
    group_by(
      .data$t, .data$regi, .data$secInd37, .data$sety, .data$fety,
      .data$emiMkt
    ) %>%
    summarise(
  ...
  pm_IndstCO2Captured <- subsector_emissions %>%
    ...
    select(
      ttot = "t", all_regi = "regi", all_enty = "sety", all_enty1 = "fety",
  ...
  for (r in getRegions(WasteShares)) {
    ...
    EmiWasteInc <- setNames(
      (dimSums(vm_incinerationEmi[, , entySEfos], dim = 3)
      - dimSums(
          mselect(vm_incinerationCCS, all_enty = c(
            entySEbio,
            entySEsyn
          )),
          dim = 3
    ...
    EmiWasteInc <- mbind(
      EmiWasteInc,
      setNames(
        EmiWasteInc[, , "Emi|CO2|Energy|Supply|Waste (Mt CO2/yr)"]
    ...
    WasteInc_CDR <- mbind(
      setNames(
        (-dimSums(mselect(vm_incinerationCCS, all_enty = entySEbio), dim = 3)
      ...
      setNames(
        (-dimSums(mselect(vm_incinerationCCS, all_enty = entySEsyn), dim = 3)
  ...
  out <- mbind(
    ...
    setNames(
      dimSums(vm_emiAllMkt[, , "co2"], dim = 3) * GtC_2_MtCO2 +
  ...
  emi.te.cp <- intersect(
    paste(getItems(p_weights_cp, dim = "all_enty", full = TRUE),
      getItems(p_weights_cp, dim = "all_enty1", full = TRUE),
      getItems(p_weights_cp, dim = "all_te", full = TRUE),
      sep = "."
    ),
  ...
  out <- mbind(
    ...
    setNames(
      (dimSums(mselect(EmiPe2Se[, , emi.te.nocp], all_enty1 = "seel"), dim = 3)
    ...
    setNames(
      (dimSums(mselect(EmiPe2Se[, , emi.te.nocp], all_enty1 = "sehe"), dim = 3)
    ...
    setNames(
      (dimSums(mselect(EmiPe2Se[, , emi.te.nocp], all_enty1 = "seh2"), dim = 3)
    ...
    setNames(
      (dimSums(mselect(EmiPe2Se[, , emi.te.nocp], all_enty1 = se_sol), dim = 3)
    ...
    setNames(
      (dimSums(mselect(EmiPe2Se[, , emi.te.nocp], all_enty1 = se_liq), dim = 3)
    ...
    setNames(
      (dimSums(mselect(EmiPe2Se[, , emi.te.nocp], all_enty1 = se_gas), dim = 3)
    ...
    setNames(
      dimSums(mselect(EmiPe2Se[, , getNames(p_weights_cp, dim = 3)], all_enty1 = "sebiochar"), dim = 3) * GtC_2_MtCO2,
  ...
  out <- mbind(
    out,
    setNames(
      dimSums(v_emiEnFuelEx[, , "co2"], dim = 3) * GtC_2_MtCO2,
  ...
  out <- mbind(
    ...
    # buildings emissions
    setNames(
      dimSums(EmiFeCarrier[, , "build"], dim = 3) * GtC_2_MtCO2
    ...
    setNames(
      (dimSums(EmiFeCarrier[, , "indst"], dim = 3)
    ...
    setNames(
      dimSums(EmiFeCarrier[, , "trans"], dim = 3) * GtC_2_MtCO2,
  ...
  out <- mbind(
    ...
    setNames(
      (dimSums(mselect(EmiFeCarrier, all_enty1 = "fesos"), dim = 3)
    ...
    setNames(
      (dimSums(mselect(EmiFeCarrier, all_enty1 = c("fehos", "fepet", "fedie")), dim = 3)
    ...
    setNames(
      (dimSums(mselect(EmiFeCarrier, all_enty1 = c("fegas", "fegat")), dim = 3)
  ...
  out <- mbind(
    ...
    # buildings
    setNames(
      (dimSums(mselect(EmiFeCarrier, all_enty1 = "fesos", emi_sectors = "build"), dim = 3)) * GtC_2_MtCO2
        # add waste emissions of demand sector
        + out[, , "Emi|CO2|Energy|Demand|Buildings|Waste (Mt CO2/yr)"],
      "Emi|CO2|Energy|Demand|Buildings|+|Solids (Mt CO2/yr)"
    ),
    setNames(
      (dimSums(mselect(EmiFeCarrier, all_enty1 = "fehos", emi_sectors = "build"), dim = 3)) * GtC_2_MtCO2,
      "Emi|CO2|Energy|Demand|Buildings|+|Liquids (Mt CO2/yr)"
    ),
    setNames(
      (dimSums(mselect(EmiFeCarrier, all_enty1 = "fegas", emi_sectors = "build"), dim = 3)) * GtC_2_MtCO2,
      "Emi|CO2|Energy|Demand|Buildings|+|Gases (Mt CO2/yr)"
    ),

    # transport
    setNames(
      (dimSums(mselect(EmiFeCarrier, all_enty1 = c("fedie", "fepet"), emi_sectors = "trans"), dim = 3)) * GtC_2_MtCO2,
      "Emi|CO2|Energy|Demand|Transport|+|Liquids (Mt CO2/yr)"
    ),
    setNames(
      (dimSums(mselect(EmiFeCarrier, all_enty1 = c("fegat"), emi_sectors = "trans"), dim = 3)) * GtC_2_MtCO2,
  ...
  SolidShareSubsec <- mbind(
    setNames(
      output[, , "FE|Industry|Steel|+|Solids (EJ/yr)"] / output[, , "FE|Industry|+|Solids (EJ/yr)"],
      "steel"
    ),
    setNames(
      output[, , "FE|Industry|Cement|+|Solids (EJ/yr)"] / output[, , "FE|Industry|+|Solids (EJ/yr)"],
      "cement"
    ),
    setNames(
      output[, , "FE|Industry|Chemicals|+|Solids (EJ/yr)"] / output[, , "FE|Industry|+|Solids (EJ/yr)"],
      "chemicals"
    ),
    setNames(
      output[, , "FE|Industry|Other Industry|+|Solids (EJ/yr)"] / output[, , "FE|Industry|+|Solids (EJ/yr)"],
      "otherInd"
    )
  ) %>%
    collapseDim()
  ...
  out <- mbind(
    ...
    subsector_emissions %>%
      ...
      group_by(
        .data$t, .data$regi, .data$secInd37, .data$fety,
        .data$origin
      ) %>%
      summarise(
        value = sum(.data$subsector_emissions)
        * as.numeric(GtC_2_MtCO2),
        .groups = "drop"
      ) %>%
      mutate(d3 = paste0(
        "Emi|CO2|pre-CCS|Energy|Demand|Industry|",
  ...
  emi.fosneg.check <- c(
    grep("Emi\\|CO2\\|Energy\\|Demand\\|Industry\\|.*Fossil", getNames(out), value = T),
  ...
  out <- mbind(
    ...
    setNames(
      (dimSums(vm_emiMacSector[, , "co2cement_process"], dim = 3)
    ...
    setNames(
      sm_capture_rate_cdrmodule * dimSums(v33_co2emi_non_atm_calcination, dim = 3) * (1 - p_share_CCS) * GtC_2_MtCO2,
  ...
  out <- mbind(
    ...
    # Industry sector
    setNames(
      out[, , "Emi|CO2|Industrial Processes|Industry|+|Cement (Mt CO2/yr)"]
      + out[, , "Emi|CO2|Industrial Processes|Industry|+|Chemicals (Mt CO2/yr)"],
      "Emi|CO2|Industrial Processes|+|Industry (Mt CO2/yr)"
    ),
    # CDR sector
    setNames(
      out[, , "Emi|CO2|Industrial Processes|CDR Sector|+|OAE (Mt CO2/yr)"],
  ...
  out <- mbind(
    out,
    setNames(
      out[, , "Emi|CO2|Waste|+|Non-Incinerated Plastic (Mt CO2/yr)"]
  ...
  out <- mbind(
    out,
    setNames(
      dimSums(sel_vm_emiAllMkt_co2, dim = 3) * GtC_2_MtCO2,
      "Emi|CO2 (Mt CO2/yr)"
    )
  )
  ...
  # pe2se emissions technologies with coupled production
  cco2.te.cp <- intersect(
    paste(getItems(p_weights_cp_cco2, dim = 3, split = TRUE, full = TRUE)$all_enty,
      getItems(p_weights_cp_cco2, dim = 3, split = TRUE, full = TRUE)$all_enty1,
      getItems(p_weights_cp_cco2, dim = 3, split = TRUE, full = TRUE)$all_te,
      sep = "."
    ),
  ...
  out <- mbind(
    ...
    # electricity
    setNames(
      (dimSums(mselect(CCO2Pe2Se[, , cco2.te.nocp], all_te = te.becc, all_enty1 = "seel", all_te = te.becc), dim = 3)
    ...
    # heat
    setNames(
      (dimSums(mselect(CCO2Pe2Se[, , cco2.te.nocp], all_te = te.becc, all_enty1 = "sehe", all_te = te.becc), dim = 3)
    ...
    # hydrogen
    setNames(
      (dimSums(mselect(CCO2Pe2Se[, , cco2.te.nocp], all_te = te.becc, all_enty1 = "seh2", all_te = te.becc), dim = 3)
    ...
    # supply-side solids
    setNames(
      (dimSums(mselect(CCO2Pe2Se[, , cco2.te.nocp], all_te = te.becc, all_enty1 = se_sol, all_te = te.becc), dim = 3)
    ...
    # supply-side liquids
    setNames(
      (dimSums(mselect(CCO2Pe2Se[, , cco2.te.nocp], all_te = te.becc, all_enty1 = se_liq, all_te = te.becc), dim = 3)
    ...
    # supply-side gases
    setNames(
      (dimSums(mselect(CCO2Pe2Se[, , cco2.te.nocp], all_te = te.becc, all_enty1 = se_gas, all_te = te.becc), dim = 3)
  ...
  out <- mbind(
    ...
    # electricity
    setNames(
      (dimSums(mselect(CCO2Pe2Se[, , cco2.te.nocp], all_te = teFosCCS, all_enty1 = "seel", all_te = teFosCCS), dim = 3)
      # coupled production technologies where electricty is first product
    ...
    # heat
    setNames(
      (dimSums(mselect(CCO2Pe2Se[, , cco2.te.nocp], all_te = teFosCCS, all_enty1 = "sehe", all_te = teFosCCS), dim = 3)
      # coupled production technologies where heat is first product
    ...
    # hydrogen
    setNames(
      (dimSums(mselect(CCO2Pe2Se[, , cco2.te.nocp], all_te = teFosCCS, all_enty1 = "seh2", all_te = teFosCCS), dim = 3)
      # coupled production technologies where hydrogen is first product
    ...
    # supply-side solids
    setNames(
      (dimSums(mselect(CCO2Pe2Se[, , cco2.te.nocp], all_te = teFosCCS, all_enty1 = se_sol, all_te = teFosCCS), dim = 3)
      # coupled production technologies where solids are first product
    ...
    # supply-side liquids
    setNames(
      (dimSums(mselect(CCO2Pe2Se[, , cco2.te.nocp], all_te = teFosCCS, all_enty1 = se_liq, all_te = teFosCCS), dim = 3)
      # coupled production technologies where liquids are first product
    ...
    # supply-side gases
    setNames(
      (dimSums(mselect(CCO2Pe2Se[, , cco2.te.nocp], all_te = teFosCCS, all_enty1 = se_gas, all_te = teFosCCS), dim = 3)
      # ecoupled production technologies where gases are first product
  ...
  out <- mbind(
    ...
    setNames(
      dimSums(vm_incinerationCCS, dim = 3) * GtC_2_MtCO2,
      "Carbon Management|Carbon Capture|+|Waste (Mt CO2/yr)"
    ),
    setNames(
      dimSums(mselect(vm_incinerationCCS, all_enty = entySEfos), dim = 3) * GtC_2_MtCO2,
      "Carbon Management|Carbon Capture|Waste|+|Fossil (Mt CO2/yr)"
    ),
    setNames(
      dimSums(mselect(vm_incinerationCCS, all_enty = entySEbio), dim = 3) * GtC_2_MtCO2,
      "Carbon Management|Carbon Capture|Waste|+|Biomass (Mt CO2/yr)"
    ),
    setNames(
      dimSums(mselect(vm_incinerationCCS, all_enty = entySEsyn), dim = 3) * GtC_2_MtCO2,
    ...
    setNames(
      -vm_emiCdrTeDetail[, , "dac"] * GtC_2_MtCO2,
  ...
  out <- mbind(
    ...
    # carbon in plastics
    setNames(
      dimSums(v37_plasticsCarbon, dim = 3) * GtC_2_MtCO2,
    ...
    setNames(
      dimSums(v37_feedstocksCarbon, dim = 3) * GtC_2_MtCO2,
  ...
  out <- mbind(
    out,
    setNames(
      dimSums(mselect(v37_plasticsCarbon, all_enty = c("seliqsyn", "sesosyn", "segasyn"))) * GtC_2_MtCO2,
      "Carbon Management|Materials|Plastics|+|Synfuels (Mt CO2/yr)"
    ),
    setNames(
      dimSums(mselect(v37_plasticsCarbon, all_enty = c("seliqbio", "sesobio", "segabio"))) * GtC_2_MtCO2,
      "Carbon Management|Materials|Plastics|+|Biomass (Mt CO2/yr)"
    ),
    setNames(
      dimSums(mselect(v37_plasticsCarbon, all_enty = c("seliqfos", "sesofos", "segafos"))) * GtC_2_MtCO2,
  ...
  out <- mbind(
    ...
    # venting of captured carbon
    setNames(
      dimSums(v_co2capturevalve, dim = 3, na.rm = TRUE) * GtC_2_MtCO2,
  ...
  out <- mbind(
    ...
    setNames(
      collapseDim(vm_co2CCUshort[, , "MeOH"]) * GtC_2_MtCO2,
  ...
  out <- mbind(
    ...
    # Pe2Se BECCS
    setNames(
      -out[, , "Carbon Management|Storage|Energy|Pe2Se|+|Biomass (Mt CO2/yr)"]
    ...
    setNames(
      -out[, , "Carbon Management|Storage|Energy|Industry|+|Biomass (Mt CO2/yr)"]
    ...
    setNames(
      -out[, , "Carbon Management|Carbon Capture|Energy|Industry|+|Synfuel (Mt CO2/yr)"] * p_share_CCS
    ...
    setNames(
      -out[, , "Carbon Management|Materials|Non-Plastics|+|Other destination (Mt CO2/yr)"] * p_share_atmosco2_plastics,
    ...
    setNames(
      (-out[, , "Carbon Management|Materials|Non-Plastics|+|Other destination (Mt CO2/yr)"]
      - out[, , "Carbon Management|Materials|Plastics|Waste|++|Other destination (Mt CO2/yr)"]) * p_share_atmosco2_plastics,
    ...
    # total DACCS
    setNames(
      -out[, , "Carbon Management|Storage|+|DAC (Mt CO2/yr)"],
      "Emi|CO2|CDR|+|DACCS (Mt CO2/yr)"
    ),

    # total Biochar
    setNames(
      emi_Biochar * GtC_2_MtCO2,
      "Emi|CO2|CDR|+|Biochar (Mt CO2/yr)"
    ),

    # total EW
    setNames(
      vm_emiCdrTeDetail[, , "weathering"] * GtC_2_MtCO2,
      "Emi|CO2|CDR|+|EW (Mt CO2/yr)"
    ),

    # OAE (Ocean Uptake reduced by unavoidable Calcination Emissions)
    setNames(
      dimSums(vm_emiCdrTeDetail[, , te_oae33], dim = 3) * GtC_2_MtCO2
    ...
    setNames(
      vm_emiCdrTeDetail[, , "oae_ng"] * GtC_2_MtCO2,
      "Emi|CO2|CDR|Net OAE|Ocean Uptake|+|Traditional Calciner (Mt CO2/yr)"
    ),
    setNames(
      vm_emiCdrTeDetail[, , "oae_el"] * GtC_2_MtCO2,
    ...
    setNames(
      dimSums(v33_co2emi_non_atm_calcination, dim = 3) * GtC_2_MtCO2
    ...
    setNames(
      out[, , "Carbon Management|Carbon Capture|Industrial Processes|CDR Sector|+|OAE (Mt CO2/yr)"]
      - out[, , "Carbon Management|Storage|Industrial Processes|CDR Sector|+|OAE (Mt CO2/yr)"],
  ...
  out <- mbind(
    out,
    setNames(
      out[, , "Emi|CO2|CDR|Net OAE|+|Ocean Uptake (Mt CO2/yr)"]
  ...
  out <- mbind(
    ...
    setNames(
      out[, , "Emi|CO2|Energy|Supply|+|Solids w/ couple prod (Mt CO2/yr)"]
    ...
    setNames(
      ...
      - out[, , "Emi|CO2|Energy|Supply|+|Biochar w/ couple prod (Mt CO2/yr)"],
      "Emi|CO2|Gross|Energy|Supply|+|Biochar (Mt CO2/yr)"
    ), # gross emissions of biochar are zero.
  ...
  out <- mbind(
    out,
    setNames(
      out[, , "Emi|CO2|Energy|Demand|+|Industry (Mt CO2/yr)"]
    ...
    setNames(
      out[, , "Emi|CO2|Energy|Demand|+|Buildings (Mt CO2/yr)"],
      "Emi|CO2|Gross|Energy|Demand|+|Buildings (Mt CO2/yr)"
    ),
    setNames(
      out[, , "Emi|CO2|Energy|Demand|+|Transport (Mt CO2/yr)"],
  ...
  out <- mbind(
    ...
    setNames(
      out[, , "Emi|CO2|Energy and Industrial Processes (Mt CO2/yr)"]
      - out[, , "Emi|CO2|CDR|+|Synthetic Fuels CCS (Mt CO2/yr)"]
        - out[, , "Emi|CO2|CDR|+|BECCS (Mt CO2/yr)"]
        - out[, , "Emi|CO2|CDR|+|Biochar (Mt CO2/yr)"],
  ...
  out <- mbind(
    ...
    # total gross non-eletric energy supply emissions
    setNames(
      out[, , "Emi|CO2|Gross|Energy|+|Supply (Mt CO2/yr)"]
  ...
  if ("ch4peatland" %in% mac.map$all_enty) {
    out <- mbind(
      ...
      setNames(
        mselect(EmiMAC, macsector = "ch4peatland"),
  ...
  if ("n2opeatland" %in% mac.map$all_enty) {
    out <- mbind(
      ...
      setNames(
        mselect(EmiMAC, macsector = "n2opeatland") * MtN2_to_ktN2O,
  ...
  out <- mbind(
    ...
    setNames(
      out[, , "Emi|GHG|Energy|+|Supply (Mt CO2eq/yr)"]
      - out[, , "Emi|CO2|CDR|BECCS|+|Pe2Se (Mt CO2/yr)"]
        - out[, , "Emi|CO2|CDR|+|Biochar (Mt CO2/yr)"]
        - out[, , "Emi|CO2|CDR|Synthetic Fuels CCS|+|Energy Supply (Mt CO2/yr)"],
    ...
    setNames(
      out[, , "Emi|GHG|Energy|+|Demand (Mt CO2eq/yr)"]
      - out[, , "Emi|CO2|CDR|Synthetic Fuels CCS|+|Industry (Mt CO2/yr)"]
        - out[, , "Emi|CO2|CDR|Synthetic Fuels CCS|+|CDR Sector (Mt CO2/yr)"]
        - out[, , "Emi|CO2|CDR|BECCS|+|Demand Side (Mt CO2/yr)"],
    ...
    setNames(
      out[, , "Emi|GHG|Energy|Demand|+|Industry (Mt CO2eq/yr)"]
      - out[, , "Emi|CO2|CDR|Synthetic Fuels CCS|+|Industry (Mt CO2/yr)"]
        - out[, , "Emi|CO2|CDR|BECCS|Demand Side|+|Industry (Mt CO2/yr)"],
      "Emi|GHG|Gross|Energy|Demand|+|Industry (Mt CO2eq/yr)"
    ),
    setNames(
      out[, , "Emi|GHG|Energy|Demand|+|Buildings (Mt CO2eq/yr)"],
      "Emi|GHG|Gross|Energy|Demand|+|Buildings (Mt CO2eq/yr)"
    ),
    setNames(
      out[, , "Emi|GHG|Energy|Demand|+|Transport (Mt CO2eq/yr)"],
      "Emi|GHG|Gross|Energy|Demand|+|Transport (Mt CO2eq/yr)"
    ),
    setNames(
      out[, , "Emi|GHG|Energy|Demand|+|CDR (Mt CO2eq/yr)"]
      - out[, , "Emi|CO2|CDR|Synthetic Fuels CCS|+|CDR Sector (Mt CO2/yr)"]
        - out[, , "Emi|CO2|CDR|BECCS|Demand Side|+|CDR Sector (Mt CO2/yr)"],
  ...
  out <- mbind(
    ...
    # total gross energy emissions
    setNames(
      out[, , "Emi|GHG|+++|Energy (Mt CO2eq/yr)"]
      - out[, , "Emi|CO2|CDR|+|Synthetic Fuels CCS (Mt CO2/yr)"]
        - out[, , "Emi|CO2|CDR|+|BECCS (Mt CO2/yr)"]
        - out[, , "Emi|CO2|CDR|+|Biochar (Mt CO2/yr)"],
  ...
  out <- mbind(
    ...
    setNames(
      out[, , "Emi|GHG|Gross|Energy|+|Supply (Mt CO2eq/yr)"]
      - out[, , "Emi|CO2|Gross|Energy|Supply|+|Electricity (Mt CO2/yr)"],
  ...
  out <- mbind(
    out,
    setNames(
      dimSums(sel_vm_emiAllMkt_CO2_ETS, dim = 3) * GtC_2_MtCO2
    ...
    setNames(
      dimSums(sel_vm_emiAllMkt_CO2_ES, dim = 3) * GtC_2_MtCO2
    ...
    setNames(
      dimSums(sel_vm_emiAllMkt_CO2_other, dim = 3) * GtC_2_MtCO2
  ...
  out <- mbind(
    ...
    # CH4
    setNames(
      dimSums(sel_vm_emiAllMkt_CH4_ETS, dim = 3),
      "Emi|CH4|++|ETS (Mt CH4/yr)"
    ),
    setNames(
      dimSums(sel_vm_emiAllMkt_CH4_ES, dim = 3),
      "Emi|CH4|++|ESR (Mt CH4/yr)"
    ),
    setNames(
      dimSums(sel_vm_emiAllMkt_CH4_other, dim = 3),
      "Emi|CH4|++|Outside ETS and ESR (Mt CH4/yr)"
    ),
    # N2O
    setNames(
      dimSums(sel_vm_emiAllMkt_N2O_ETS, dim = 3) * MtN2_to_ktN2O,
      "Emi|N2O|++|ETS (kt N2O/yr)"
    ),
    setNames(
      dimSums(sel_vm_emiAllMkt_N2O_ES, dim = 3) * MtN2_to_ktN2O,
      "Emi|N2O|++|ESR (kt N2O/yr)"
    ),
    setNames(
      dimSums(sel_vm_emiAllMkt_N2O_other, dim = 3) * MtN2_to_ktN2O,
  ...
  out <- mbind(
    out,
    setNames(
      (vm_co2eq - vm_emiMacSector[, , "co2luc"] * (as.vector(cm_multigasscen) != 3)) * GtC_2_MtCO2,
  ...
  out <- mbind(
    out,
    setNames(
      out[, , "Emi|GHG (Mt CO2eq/yr)"] - out[, , "Emi|CO2|+|Land-Use Change (Mt CO2/yr)"],
  ...
  if ("DEU" %in% getRegions(vm_co2eq)) {
    ...
    out <- mbind(out, new.magpie(getRegions(vm_co2eq), getYears(vm_co2eq),
      "Emi|CO2|Energy|Supply|Electricity|Autoproducer (Mt CO2/yr)",
      fill = 0
    ))
